...
export const createEditorOps = ({
  ...
  const setCaretOffset = (offset, updatePreferred, setSelectionOffsets) => {
    setSelectionOffsets(offset, offset, updatePreferred);
  };

  const insertText = (text, setSelectionOffsets) => {
    if (!text) {
      return;
    }
  ...
  const insertTextWithBreaks = (text, setSelectionOffsets) => {
    if (!text) {
      return;
    }
    ...
    segments.forEach((segment, index) => {
      if (segment.length > 0) {
        insertText(segment, setSelectionOffsets);
      }

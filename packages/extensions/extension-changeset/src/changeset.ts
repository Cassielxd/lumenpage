...
export class ChangeSet<Data = any> {
  ...
  addSteps(newDoc: Node, maps: readonly StepMap[], data: Data | readonly Data[]): ChangeSet<Data> {
    ...
    const newChanges = mergeAll(stepChanges, this.config.combine);
    const changes = Change.merge(this.changes, newChanges, this.config.combine);
    let updated: Change<Data>[] = changes as Change<Data>[];

    // Minimize changes when possible
    for (let i = 0; i < updated.length; i++) {
      const change = updated[i];
      ...
      const diff = computeDiff(this.config.doc.content, newDoc.content, change, this.config.encoder);
  ...
  map(f: (range: Span<Data>) => Data): ChangeSet<Data> {
    const mapSpan = (span: Span<Data>) => {
      const newData = f(span);
  ...
  changedRange(b: ChangeSet, maps?: readonly StepMap[]): {from: number, to: number} | null {
    ...
    const touched = maps && touchedRange(maps);
    const moved = touched ? (touched.toB - touched.fromB) - (touched.toA - touched.fromA) : 0;
...
function touchedRange(maps: readonly StepMap[]) {
  const b = endRange(maps);
  if (!b) return null;
  const a = endRange(maps.map((m) => m.invert()).reverse())!;

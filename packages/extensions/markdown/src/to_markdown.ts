import {Node, Mark} from "lumenpage-model"
...
const escapeHtmlAttr = (value: string) => escapeHtmlText(value).replace(/"/g, "&quot;");
...
export const defaultMarkdownSerializer = new MarkdownSerializer({
  ...
  table(state, node) {
    ...
    node.forEach((row) => {
      ...
      row.forEach((cell) => {
        ...
        const cellText = escapeHtmlText(cell.textBetween(0, cell.content.size, "\n"))
        state.write(`<${tag}${attrs.join("")}>${cellText}</${tag}>`)
      })
      state.write("</tr>")
    })
    state.write("\n</table>")
  ...
  video(state, node) {
    ...
    if (!src) {
      state.write("<video controls></video>")
      state.closeBlock(node)
      return
    }
    const width = node.attrs?.width ? ` width="${escapeHtmlAttr(String(node.attrs.width))}"` : ""
    const height = node.attrs?.height ? ` height="${escapeHtmlAttr(String(node.attrs.height))}"` : ""
    if (node.attrs?.embed) {
      state.write(
        `<iframe src="${escapeHtmlAttr(src)}"${width}${height} allowfullscreen></iframe>`
      )
      state.closeBlock(node)
      return
    }
    const poster = node.attrs?.poster
      ? ` poster="${escapeHtmlAttr(String(node.attrs.poster))}"`
      : ""
    state.write(
      `<video src="${escapeHtmlAttr(src)}"${poster}${width}${height} controls></video>`
    )
    state.closeBlock(node)
  },
  ...
  underline: {open: "<u>", close: "</u>", mixable: false},

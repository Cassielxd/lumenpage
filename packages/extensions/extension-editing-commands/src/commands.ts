...
import { backspaceEmptyListItem, splitListItem } from "lumenpage-extension-list-item";
import { toggleTaskItemChecked } from "lumenpage-extension-task-item";
import {
  addTableRowAfter,
  addTableRowBefore,
  deleteTableRow,
...
} from "lumenpage-extension-table";
export {
  addTableRowAfter,
  addTableRowBefore,
  deleteTableRow,
...
export const createViewCommands = () => {
  const isListItemTypeName = (name) => name === "listItem" || name === "taskItem";
  const getExpectedItemTypeNameForList = (listTypeName) => (listTypeName === "taskList" ? "taskItem" : "listItem");
  ...
  const convertListItemsForListType = (tr, schema, listPos, listNode, targetListTypeName) => {
    ...
    listNode.forEach((child, childOffset) => {
      if (!isListItemTypeName(child.type?.name) || child.type === targetItemType) {
        return;
      }
  ...
  const toggleList = (nodeName) => (state, dispatch, view) => {
    ...
    for (let depth = $from.depth; depth > 0; depth -= 1) {
      ...
      if (!isListItemTypeName(node.type.name)) {
        continue;
      }
      ...
      if (
        ...
        tr = convertListItemsForListType(tr, state.schema, listPos, parent, nodeName);
        dispatch(tr.scrollIntoView());
        return true;
      }
      ...
      range = $from.blockRange($to, (n) => isListItemTypeName(n.type.name));
      break;
    }
    if (range) {
  ...
  return {
    ...
    toggleBold: toggleMarkByName("bold"),
    toggleItalic: toggleMarkByName("italic"),
    ...
    insertHorizontalRule: insertNode("horizontalRule"),
    insertHardBreak: insertNode("hardBreak"),
    insertPageBreak: insertNode("pageBreak"),

...
import { Plugin, type Command } from "lumenpage-state"
...
export function keydownHandler(bindings: {[key: string]: Command}): (view: EditorView, event: KeyboardEvent) => boolean {
  ...
  return function(view, event) {
    ...
    const dispatch = typeof view.dispatch == "function" ? view.dispatch.bind(view) : undefined
    let name = keyName(event), baseName, direct = map[modifiers(name, event)]
    if (direct && direct(state as any, dispatch as any, view)) return true
    // A character key
    if (name.length == 1 && name != " ") {
      if (event.shiftKey) {
        ...
        if (noShift && noShift(state as any, dispatch as any, view)) return true
      }
      if ((event.altKey || event.metaKey || event.ctrlKey) &&
        ...
        if (fromCode && fromCode(state as any, dispatch as any, view)) return true
      }
    }
    return false
  }

...
const findNearestCellPos = (doc, pos, dir = 1) => {
  ...
  doc.descendants((node, nodePos) => {
    if (!isTableCellTypeName(node?.type?.name)) {
      return true;
    }
...
const resolveCellBoundary = (doc, pos, dir = 1) => {
  ...
  if (!isTableCellTypeName($pos.nodeAfter?.type?.name)) {
    return null;
  }
  return $pos;
};

const resolveCellNodeAt = ($pos) => (isTableCellTypeName($pos?.nodeAfter?.type?.name) ? $pos.nodeAfter : null);
...
export class CellSelection extends Selection {
  constructor($anchorCell, $headCell = $anchorCell) {
    ...
    if (!anchorCell || !headCell) {
      throw new RangeError("CellSelection endpoints must point at table cell boundaries.");
    }
  ...
  toJSON() {
    return { type: "tableCell", anchor: this.anchor, head: this.head };
  }
...
try {
  Selection.jsonID("tableCell", CellSelection);
} catch (_error) {

...
const createEmptyCell = (schema) => {
  ...
  throw new Error("Cannot create tableCell with the current schema.");
};
...
const findAncestorDepthByTypeNames = ($pos, typeNames) => {
...
const isAtCellBoundary = ($head, direction) => {
  const cellDepth = findAncestorDepthByTypeNames($head, ["tableCell", "tableHeader"]);
...
const findTableContext = (state) => {
  ...
  let rowIndex = 0;
  if (tableDepth + 1 <= $from.depth && $from.node(tableDepth + 1)?.type?.name === "tableRow") {
    rowIndex = $from.index(tableDepth);
  }

  let colIndex = 0;
  if (tableDepth + 2 <= $from.depth && isTableCellTypeName($from.node(tableDepth + 2)?.type?.name)) {
...
export const addTableColumnAfter = (state, dispatch) =>
  updateRows(state, dispatch, (rows, cols, context) => {
    ...
    return rows.map((row) => {
      ...
      return state.schema.nodes.tableRow.create(row.attrs, cells, row.marks);
    });
  });
...
export const deleteTableColumn = (state, dispatch) =>
  updateRows(state, dispatch, (rows, cols, context) => {
    ...
    for (let r = 0; r < grid.matrix.length; r += 1) {
      ...
      nextRows[entry.row] = state.schema.nodes.tableRow.create(rowNode.attrs, cells, rowNode.marks);
    }
...
export const deleteTableCellSelection = (state, dispatch) => {
  ...
  for (let rowIndex = 0; rowIndex < context.table.childCount; rowIndex += 1) {
    ...
    nextRows.push(state.schema.nodes.tableRow.create(row.attrs, cells, row.marks));
  }
...
export const mergeTableCellRight = (state, dispatch) =>
  updateRows(state, dispatch, (rows, cols, context) => {
    ...
    const nextRows = rows.slice();
    nextRows[anchor.rowIndex] = state.schema.nodes.tableRow.create(row.attrs, cells, row.marks);
    return nextRows;
  });
...
export const splitTableCell = (state, dispatch) =>
  updateRows(state, dispatch, (rows, cols, context) => {
    ...
    if (colspan > 1) {
      ...
      const nextRows = rows.slice();
      nextRows[targetRowIndex] = state.schema.nodes.tableRow.create(row.attrs, cells, row.marks);
      return nextRows;
    }
    ...
    if (rowspan > 1) {
      ...
      nextRows[targetRowIndex] = state.schema.nodes.tableRow.create(
        row.attrs,
      ...
      nextRows[rowToInsertIndex] = state.schema.nodes.tableRow.create(
        targetRow.attrs,
        targetCells,
        targetRow.marks
      );

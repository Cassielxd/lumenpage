...
const resolveHeadingLineHeight = (line: any, fallback: number) =>
  Number.isFinite(line?.lineHeight) && Number(line.lineHeight) > 0
    ? Number(line.lineHeight)
    : Math.max(1, Number(fallback) || 1);

const measureHeadingLinesHeight = (lines: any[], fallbackLineHeight: number) => {
  ...
  for (const line of lines) {
    const lineHeight = resolveHeadingLineHeight(line, fallbackLineHeight);
...
const getFittableHeadingLineCount = (
  lines: any[],
...
const buildHeadingRuns = (node: any, settings: any, registry: any) => {
  const level = Math.max(1, Math.min(3, Number(node.attrs?.level) || 1));
...
export const headingRenderer = {
  ...
  toRuns(node: any, settings: any, registry: any) {
    return buildHeadingRuns(node, settings, registry);
  },
  measureBlock(ctx: any) {
    const { node, settings, registry, measureTextWidth } = ctx || {};
    const runs = buildHeadingRuns(node, settings, registry);
    const maxWidth = Math.max(
      0,
      Number(settings?.pageWidth || 0) -
        Number(settings?.margin?.left || 0) -
        Number(settings?.margin?.right || 0),
    );
  ...
  paginateBlock(ctx: any) {
    ...
    if (remainingLines.length === 0) {
      return {
        slice: {
          ...
          nextCursor: null,
          meta: {
            source: "heading-modern-paginate",
            exhausted: true,
          },
        },
        nextCursor: null,
    ...
    const fittableCount = getFittableHeadingLineCount(
      remainingLines,
    ...
    const fragmentIdentity = `heading:${measured?.nodeId ?? measured?.startPos ?? 0}`;
    const continuation = {
      fromPrev: startIndex > 0,
      hasNext,
      rowSplit: false,
    ...
    const visibleLines = cloneHeadingSliceLines(allLines, startIndex, endIndex, continuation);
    const nextCursor =
      hasNext
        ? {
            nodeId: measured?.nodeId ?? null,
    ...
    return {
      slice: {
        ...
        endPos: hasNext
          ? Number(measured?.startPos || 0) +
            Number(allLines[Math.max(startIndex, endIndex - 1)]?.end || 0)

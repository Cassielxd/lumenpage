import { defaultBookmarkRenderer as baseBookmarkRenderer } from "lumenpage-render-engine";
...
export const createDefaultBookmarkNodeView = (node: any, view: any) => {
  ...
  container.className = "lumenpage-bookmark-overlay";
  ...
  const updateLink = (nextNode: any) => {
    ...
    link.href = href || "#";
    link.title = String(nextNode.attrs?.title || nextNode.attrs?.href || "Reference");
...
export const bookmarkRenderer = {
  ...baseBookmarkRenderer,
  createNodeView: createDefaultBookmarkNodeView,
};

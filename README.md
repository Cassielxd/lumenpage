...
- `packages/view-canvas` (`lumenpage-view-canvas`): canvas renderer + input/selection handling, plus embedded core/layout modules (`src/core`, `src/layout-pagination`).
...
- `packages/view-canvas/src/core` does not depend on DOM APIs.
- `packages/kit-basic` creates the default schema and a default node renderer registry.
- `packages/view-canvas` provides DOM/canvas implementations and exports core + view helpers.
